  ...
  try {
    ...
    if (!isCheckCall) {
      ...
      if (payhipData.data.uses >= 1) {
        return new Response(JSON.stringify({ error: 'This code has already been used.' }), { status: 401, headers });
      }
    }
    let messages;
    if (reviewMode && draftLetter) {
      messages = [{ role: 'user', content: `Review and improve this demand letter. Return ONLY the improved letter:\n\n${draftLetter}` }];
    } else {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 1500, system: (!isCheckCall && systemPrompt) ? systemPrompt : undefined, messages }),
    });
    if (!response.ok) {
